...
export default function Udhaar() {
  ...
  const handleSettlePayment = async () => {
    ...
    setIsSettling(true);
    try {
      // Update bill status to 'paid' securely
      const { error } = await supabase
        ...
        .eq("tenant_id", currentTenantId); // Extra layer of multi-tenant security
      ...
      <Dialog open={!!selectedBill} onOpenChange={(open) => !open && setSelectedBill(null)}>
        <DialogContent className="sm:max-w-md top-[30%] translate-y-[-30%] rounded-xl">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button onClick={handleSettlePayment} disabled={isSettling} className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white">
